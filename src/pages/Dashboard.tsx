// // src/pages/Dashboard/Dashboard.tsx
// import StatsCard from '../Componets/StatsCard';
// import TaskCard from '../Componets/TaskCard';
// import { useTasks } from '../hooks/useTasks';

// export default function Dashboard() {
//   const { tasks, stats, markCompleted, deleteTask } = useTasks();
//   const recent = tasks.slice(0, 5);

//   return (
//     <div className="max-w-5xl mx-auto p-4 space-y-6">
//       <h1 className="text-2xl font-bold">Personal Productivity Dashboard</h1>

//       <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
//         <StatsCard label="Total tasks" value={stats.total} accent="blue" />
//         <StatsCard label="Completed" value={stats.completed} accent="green" />
//         <StatsCard label="Pending" value={stats.pending} accent="yellow" />
//         <StatsCard label="Progress" value={`${stats.progress}%`} accent="blue" />
//       </div>

//       <div>
//         <h2 className="text-xl font-semibold mb-3">Recent tasks</h2>
//         {recent.length === 0 ? (
//           <p className="text-sm opacity-70">No tasks yet. Create your first task to get started.</p>
//         ) : (
//           <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
//             {recent.map((t) => (
//               <TaskCard key={t.id} task={t} onComplete={markCompleted} onDelete={deleteTask} />
//             ))}
//           </div>
//         )}
//       </div>
//     </div>
//   );
// }

// src/pages/Dashboard/Dashboard.tsx
import StatsCard from '../Componets/StatsCard';
import TaskCard from '../Componets/TaskCard';
import { useTasks } from '../hooks/useTasks';

export default function Dashboard() {
  const { tasks, stats, markCompleted, deleteTask } = useTasks();
  const recent = tasks.slice(0, 5);

  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-8">
      {/* Header */}

      < div className="flex items-center gap-10">            <div className="flex items-center gap-3 bg-white/10 px-3 py-2 rounded-full backdrop-blur-md border border-white/20 hover:bg-white/20 transition">
            <img
              src="data:image/jpeg;base64,"
              alt="User Profile"
              className="w-50 h-50 object-cover border-2 border-green-400"
            />
            <span className="hidden sm:block text-black font-semibold text-3xl">
              Anshika Ojha
            </span>
          </div>
</div>
      <h1 className="text-4xl font-extrabold text-blue-700 dark:text-blue-400 tracking-tight mb-8 text-center">
        Personal Productivity Dashboard
      </h1>

      {/* Stats Section */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatsCard
          label="Total tasks"
          value={stats.total}
          accent="blue"
          className="hover:scale-105 transition-transform duration-300 shadow-lg rounded-xl"
        />
        <StatsCard
          label="Completed"
          value={stats.completed}
          accent="green"
          className="hover:scale-105 transition-transform duration-300 shadow-lg rounded-xl"
        />
        <StatsCard
          label="Pending"
          value={stats.pending}
          accent="yellow"
          className="hover:scale-105 transition-transform duration-300 shadow-lg rounded-xl"
        />
        <StatsCard
          label="Progress"
          value={`${stats.progress}%`}
          accent="purple"
          className="hover:scale-105 transition-transform duration-300 shadow-lg rounded-xl"
        />
      </div>

      {/* Recent Tasks Section */}
      <div className="mt-10 bg-white dark:bg-gray-800 shadow-xl rounded-xl p-6 border border-gray-200 dark:border-gray-700">
        <h2 className="text-2xl font-semibold mb-4 text-gray-700 dark:text-gray-200">
          Recent Tasks
        </h2>

        {recent.length === 0 ? (
          <p className="text-sm text-gray-500 dark:text-gray-400 italic">
            No tasks yet. Create your first task to get started.
          </p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {recent.map((t) => (
              <TaskCard
                key={t.id}
                task={t}
                onComplete={markCompleted}
                onDelete={deleteTask}
                className="hover:shadow-lg hover:-translate-y-1 transition-all duration-300 rounded-lg"
              />
            ))}
          </div>
        )}
      </div>
    </div>
  );
}